class TopicsController < ApplicationController
  before_action :set_topic, only: [:show, :edit, :update, :destroy]

  # GET /topics
  # GET /topics.json
  def index

    if (params[:sort]=="vote")
      @params = "vote"
      @topics = Topic.select("topics.*, count(votes.topic_id) as vote_count")
        .joins("left outer join votes on topics.id=votes.topic_id")
        .group("votes.topic_id, topics.title, topics.id")
        .order("vote_count desc")

    else
      @params=params[:sort]
      @topics = Topic.order(@params)

    end
  end

  # GET /topics/1
  # GET /topics/1.json
  def show
  end

  # GET /topics/new
  def new
    @topic = Topic.new
  end

  # GET /topics/1/edit
  def edit
  end

  # POST /topics
  # POST /topics.json
  def create
    @topic = Topic.new(topic_params)

    respond_to do |format|
      if @topic.save
        format.html { redirect_to topics_url, notice: 'What an excellent topic that is.' }
        format.json { render :show, status: :created, location: @topic }
      else
        format.html { render :new }
        format.json { render json: @topic.errors, status: :unprocessable_entity }
      end
    end
  end

  # PATCH/PUT /topics/1
  # PATCH/PUT /topics/1.json
  def update
    respond_to do |format|
      if @topic.update(topic_params)
        format.html { redirect_to topics_url, notice: 'You made this topic even better.' }
        format.json { render :show, status: :ok, location: @topic }
      else
        format.html { render :edit }
        format.json { render json: @topic.errors, status: :unprocessable_entity }
      end
    end
  end

  # DELETE /topics/1
  # DELETE /topics/1.json
  def destroy
    @topic.destroy
    respond_to do |format|
      format.html { redirect_to topics_url, notice: 'Shed a tear for the loss of that beautiful topic.' }
      format.json { head :no_content }
    end
  end

  def upvote
    @topic = Topic.find(params[:id])
    @topic.votes.create
    redirect_to(topics_path(sort: params[:sort]))
  end

  def downvote
    @topic = Topic.find(params[:id])
      if @topic.votes.count > 0
        @topic.votes.last.destroy
        redirect_to(topics_path(sort: params[:sort]))
      else
      respond_to do |format|
        format.html { redirect_to (topics_path(sort: params[:sort])), notice: "You can't be less popular than zero." }
      end
    end
  end


  private
    # Use callbacks to share common setup or constraints between actions.
    def set_topic
      @topic = Topic.find(params[:id])
    end

    # Never trust parameters from the scary internet, only allow the white list through.
    def topic_params
      params.require(:topic).permit(:title, :description)
    end
end
